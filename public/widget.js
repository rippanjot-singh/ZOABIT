(function () {
  ...
  if (!currentScript) { console.error("SiteBot: currentScript is null."); return; }

  const chatbotId = currentScript.getAttribute("data-id");
  if (!chatbotId) { console.error('SiteBot: Add data-id="YOUR_CHATBOT_ID" to the script tag.'); return; }
  ...
  const HISTORY_KEY = `sitebot_history_${chatbotId}`;
    ...
    appDiv.innerHTML = `
      <div class="chat-window" id="sb-win" style="display: none;">
        ...
        <div class="chat-footer" id="sb-footer">
          ...
          <div class="powered-by">Powered by SiteBot</div>
        </div>
      </div>`;
  ...
  function init() {
    ...
    container.id = `sitebot-${chatbotId}`;
    ...
    fetch(`${backendOrigin}/api/chatbot/config/${chatbotId}`)
      ...
      .finally(() => {
        ...
        if (v.length > 0 && !v.includes(host) && host !== 'localhost' && host !== '127.0.0.1') {
          console.warn('SiteBot: Domain not authorized for this widget.');
